...
namespace FAMS_API.Controllers
{
    ...
    public class UomController : Controller
    {
        ...
        public ResultData List(IndexParams request = null)
        {
            ...
            try
            {
                string query = @"SELECT A.*
                                    ...
                                    ORDER BY {3} {2} {1}";
                string whereQuery = "";
                string totalQuery = @"SELECT COUNT(A.Id) From [Uom] A 
                                    WHERE 1=1 {0}";
                ...
                var data = db.Database.SqlQuery<UomModel>(fQuery, parameters.ToArray()).ToList();
        ...
        public ResultData GetData(long id)
        {
            ...
            try
            {
                var header = new UomModel();

                if (id != 0)
                    header = new UomViewModel(db, id);
        ...
        [HttpPost]
        public ResultData Save(UomModel modelData)
        {
            var result = new ResultData();
            bool success = false;
            string message = "";
            ...
            try
            {
                ...
                {
                    var existedData = db.UoM.Where(r => r.Code == modelData.Code && r.IsDeleted == "N").ToArray();

                    if (existedData != null && existedData.Length > 0 && modelData.mode == Constants.FORM_MODE_CREATE)
                        throw new Exception(string.Format(Resources.EXIST_DATA, "Uom " + modelData.Code));
                ...
                {
                    model = db.UoM.Find(modelData.Id);
                    ...
                    {
                        if (UUtilsApi.HasTransaction(typeof(UoM), model.Id, out message))
                            throw new Exception(string.Format(Resources.USED_ON_TRANSACTION, "Uom ", "code", message));
        ...
        {
            public UomModel header { get; set; }
        }
    }
}
