...
namespace FAMS_API.Controllers
{
    ...
    [ApiController]
    [ApiAuthorize]
    public class LocationSAPController : Controller
    {
        ...
        public ResultData List(IndexParams request = null)
        {
            ...
            try
            {
                string query = @"SELECT A.*
                                    ...
                                    ORDER BY {3} {2} {1}";
                string whereQuery = "";
                string totalQuery = @"SELECT COUNT(A.Id) From [LocationSAP] A 
                                    WHERE 1=1 {0}";
                ...
                var data = db.Database.SqlQuery<LocationSAPModel>(fQuery, parameters.ToArray()).ToList();
        ...
        public ResultData GetData(long id)
        {
            ...
            try
            {
                var header = new LocationSAPModel();

                if (id != 0)
                    header = new LocationSAPViewModel(db, id);
        ...
        [HttpPost]
        public ResultData Save(LocationSAPModel modelData)
        {
            var result = new ResultData();
            bool success = false;
            string message = "";
            ...
            try
            {
                ...
                LocationSAP model = new LocationSAP();

                if (modelData.mode == Constants.FORM_MODE_CREATE)
                {
                    var existedData = db.LocationSAP.Where(r => r.SAPCompanyCode == modelData.SAPCompanyCode && r.Location == modelData.Location && r.IsDeleted == "N").ToArray();

                    if (existedData != null && existedData.Length > 0 && modelData.mode == Constants.FORM_MODE_CREATE)
                        throw new Exception(string.Format(Resources.EXIST_DATA, "Location SAP " + modelData.SAPCompanyCode + " - " + modelData.Location));
                ...
                {
                    model = db.LocationSAP.Find(modelData.Id);
                    ...
                    {
                        if (UUtilsApi.HasTransaction(typeof(LocationSAP), model.Id, out message))
                            throw new Exception(string.Format(Resources.USED_ON_TRANSACTION, "Location SAP", "code", message));
                ...
                {
                    model.SAPCompanyCode = modelData.SAPCompanyCode;
                    model.Location = modelData.Location;
                    model.Plant = modelData.Plant;
                    model.BA = modelData.BA;
                }
        ...
        {
            public LocationSAPModel header { get; set; }
        }
    }
}
