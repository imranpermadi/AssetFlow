    ...
    public class DepartmentController : Controller
    {
        ...
        public ResultData List(IndexParams request = null)
        {
            ...
            try
            {
                string query = @"SELECT A.*
                                    ...
                                    ORDER BY {3} {2} {1}";
                string whereQuery = "";
                string totalQuery = @"SELECT COUNT(A.Id) From [Department] A 
                                    WHERE 1=1 {0}";
                ...
                var data = db.Database.SqlQuery<DepartmentModel>(fQuery, parameters.ToArray()).ToList();
        ...
        public ResultData Save(DepartmentModel modelData)
        {
            ...
            try
            {
                ...
                {
                    var existedData = db.Department.Where(r => r.Code == modelData.Code && r.IsDeleted == "N").ToArray();

                    if (existedData != null && existedData.Length > 0 && modelData.mode == Constants.FORM_MODE_CREATE)
                        throw new Exception(string.Format(Resources.EXIST_DATA, "Department " + modelData.Code));
                ...
                else
                {
                    ...
                    {
                        if (UUtilsApi.HasTransaction(typeof(Department), model.Id, out message))
                            throw new Exception(string.Format(Resources.USED_ON_TRANSACTION, "Department ", "code", message));
