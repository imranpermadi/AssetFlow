                    ...
                    <div class="grid">
                        <div class="col-6">
                            <div class="field grid">
                                ...
                                <div class="col-8 md-8">
                                    <InputText name="Code" v-model="header.Code" type="text" :placeholder="$t('code')"
                                        v-if="header" :class="{ 'p-invalid': submitted && !header.Code }"
                                        :disabled="mode != this.$FORM_MODE_CREATE"
                                        :readonly="mode != this.$FORM_MODE_CREATE" />
                                    <Skeleton v-else-if="!header" />
                                    <small v-if="header && !header.Code && submitted && mode != this.$FORM_MODE_DELETE"
                            ...
                            <div class="field grid">
                                ...
                                <div class="col-8 md-8">
                                    <InputText name="Description" v-model="header.Description" required="true"
                                        type="text" :placeholder="$t('description')" v-if="header"
                                        :class="{ 'p-invalid': submitted && !header.Description }"
                                        :disabled="mode != this.$FORM_MODE_CREATE && mode != this.$FORM_MODE_EDIT" />
                                    <Skeleton v-else-if="!header" />
                                    <small
                                        v-if="header && !header.Description && submitted && mode != this.$FORM_MODE_DELETE"
...
export default {
    ...
    data() {
        return {
            indexName: "CertificateTypeIndex",
            url: this.$API_URL + "certificatetype/",
            header: null,
            mode: "",
            submitted: false,
            id: 0,
    ...
    methods: {
            ...
            this.$axios.post(this.url, this.header)
                .then((response) => {
                    if (response.data.success) {
                        self.$router.push({ name: self.indexName, params: { showToast: true, severity: 'success', summary: 'Data Saved', detail: "Certificate Type " + self.header.Code + " has been saved successfully", life: 3000 } });
                    } else {
        ...
        backToIndex() {
            this.$router.push({ path: "/CertificateType/Index" });
        },
        toggle(event) {
        ...
        confirm(event) {
            this.submitted = true;
            if (!this.header.Code || !this.header.Description) {
                return;
            } else {
