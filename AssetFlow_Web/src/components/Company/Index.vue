    ...
    <Card>
        <template #title>
            <Toolbar>
                ...
                <template #right>
                    <Button icon="pi pi-file-pdf" class="p-button-danger mr-2" @click="downloadPrint1('PDF')" />
                    <Button icon="pi pi-file" class="p-button-primary mr-2" @click="downloadPrint1('WORD')" />
                    <Button icon="pi pi-file-excel"  class="p-button-success mr-2" @click="downloadPrint1('EXCEL')" />
    ...
    export default {
        ...
        methods: {
            downloadPrint(type){
                this.$emit('block-ui');
                var self = this;
                    this.$axios.get(this.$API_URL + "Company/" + "downloadprint", {
                    responseType: 'blob',
                    params : {
                        type : type,
                    }  
                    ...
                    .catch(function (err) {
                        self.$parent.$toast.add({ severity: 'error', summary: "Error", detail: "File not found" + err, life: 120000});
                        self.blocked = false;
                        self.$emit('unblock-ui');
            ...
            downloadPrint1(type){
                ...
                this.$axios.post(this.$API_URL + "Company/" + "downloadprint",self.lazyParams,{
                    responseType: 'blob'
                })
                .then((response) => {
